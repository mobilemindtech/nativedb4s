...
extension (r: RowResult)
  def getOrNull[T <: ScalaTypes](
      index: Int | String
  )(using TypeConverter[T], Zone): T | Null =
    getAs[T](index) match
      case Some(v) => v
      case None    => null

  def getAs[T <: ScalaTypes](
      index: Int | String
  )(using tc: TypeConverter[T], z: Zone): Option[T] =
        ...
        case _ =>
          ...
          then data.addOne(col.name -> this.getOrNull[String](i))
            ...
            throw MySqlException(
              s"wrong mysql data type ${col.typ} for column ${col.name}",
              -1
            )
